<template>
  <div class="table-toolbar">
    ...
    <div v-if="getBindValues.showRightTools" class="flex items-center table-toolbar-right">
      <!--顶部右侧区域-->
...
<script lang="ts">
  ...
  import { getViewportOffset } from '../../../../../../MFS-XANGYA3/xy3-mfs-web/src/utils/domUtils.ts';
  import { useWindowSizeFn } from '../../../../../../MFS-XANGYA3/xy3-mfs-web/src/hooks/event/useWindowSizeFn.ts';
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      function updateCheckedRowKeys(rowKeys) {
        // 如果有rowKeyName， 那么根据rowKeyName过滤出当前列数据
        if (unref(getProps).rowKeyName) {
          const finalRowKeys: any = [];
          const rowKeyName = unref(getProps).rowKeyName;
          const dataSource = unref(getDataSource());
          rowKeys.map((item) => {
            const row = find(dataSource, (row: any) => row[rowKeyName].toString() === item.toString());
            if (row) {
              finalRowKeys.push(unref(row));
            }
          });
          emit('update:select-row', finalRowKeys);
        } else {
      ...
      const getBindValues = computed(() => {
        ...
        return {
          ...
          bordered: true,
          singleLine: false,
          // 'max-height': getProps.value.autoHeight ? 'auto' : maxHeight,
        };
      });
